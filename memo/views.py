...
from forms import MemoForm, SearchForm
...
def index():
    ...
    if keyword is not None:
        # keywordをタイトルに含むメモを取得
        memos = Memo.query.filter_by(user_id=current_user.id).filter(Memo.title.like('%' + keyword + '%')).all()
    else:
        # メモ全件取得
        memos = Memo.query.filter_by(user_id=current_user.id).all()
    
    # 画面遷移
    return render_template("memo/index.html", memos=memos, form=form)
